...
// modeling bond future
class BondFuture : public Future {
public:
	BondFuture(string _productId, Bond* _bond, date _maturityDate);

	// Return the underlying Bond of the future
	Bond* GetUnderlying() const;
private:
	Bond* underlyingBond;
};
...
//*********Future Functions Implementation*************//
Future::Future(string _productId, string _ticker, date _maturityDate) :
...
{
	output << future.GetTicker() << " " << future.GetMaturityDate();
	return output;
}

// for the EuroDollar future
// delegate to the Future class constructor
// specify the ticker as euroDollar
EuroDollarFuture::EuroDollarFuture(string _productId, date _maturityDate):
	Future(_productId, "EuroDollar",_maturityDate) {}

// for the BondFuture
// we bind a Bond to it
// underlying ticker: we directly refer to the ticer of the bond
BondFuture::BondFuture(string _productId, Bond* _bond, date _maturityDate) :
	Future(_productId, _bond->GetTicker(), _maturityDate) {}

// the functionality to bring the underlying bond
Bond* BondFuture::GetUnderlying() const
{
